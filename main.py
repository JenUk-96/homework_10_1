...
def file_type_option() -> list:
    ...
    if data_types[data_type_chosen] == "CSV":
        transaction_list_raw = read_transactions_pd(file_path_csv)
    elif data_types[data_type_chosen] == "XLSX":
        transaction_list_raw = read_transactons_xtml(path_file_xlsx)
    elif data_types[data_type_chosen] == "JSON":
        transaction_list_raw = financial_transactions(path_file_json)
...
def filter_rub_option(transaction_list: list) -> list:
    ...
    rub_trans_chosen = input("Выводить только рублевые транзакции? Да/Нет\n")
